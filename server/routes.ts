...
// VM Inventory routes
  ...
  app.get('/api/consumables/:id/assignments', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.post('/api/consumables/:id/assign', async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const assignmentData = req.body;
      ...
      const assignment = await storage.assignConsumable(id, assignmentData);
      
      // Update the consumable quantity
      await storage.updateConsumable(id, {
      ...
      console.error('Error assigning consumable:', error);
      res.status(500).json({ error: 'Failed to assign consumable' });
    }
  });
